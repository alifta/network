...
def db_execute(db, query):
    """
    Execute a sqlite query such as create table
    """
    try:
...
def db_execute_many(db, query, data):
    """
    Execute multiple sqlite query such as insert many
    """
    try:
        con = sqlite3.connect(db)
        cur = con.cursor()
...
def db_select_df(db, query):
    """
    Execute a SELECT sqlite query and return result as dataframe
    """
    try:
        con = sqlite3.connect(db)
...
def db_row_count(db, table, output=False):
    """
    COUNT rows of a table in sqlite database
    """
    try:
        con = sqlite3.connect(db)
        cur = con.cursor()
...
def pd_unlimit():
    """
    Remove line limit (i.e. row) of priting pandas dataframe
    """
...
def path_edit(
    ...
    """
    Edit paths (input is a a list of file names) with following format:
    <folder_name>_<folder_label>/<file_name>_<file_label>.<extension>
    """
...
def folder_walk(path, ext='', save=False):
    """
    Iterate through the files inside the input folder
    Return a list of all the files with specified extension
    """
    f = []
...
def folder_walk_folder(path, name=False, relative=False, save=False):
    """
    Iterate through the sub-folders inside the input folder
    """
    f = []
    for root, dirs, files in os.walk(path):
        for dir in dirs:
...
def file_line_count(file_name):
    """
    Count number of lines (i.e. entries) in the input file
    """
...
def colors_create(number_of_colors=1, color_map='Wistia', output=False):
    """
    Create a list of colors from the specified spectrum e.g. Wistia, cold or hot
    """
    colors = []
...
def dict_save(data, file_name='data', method='n', sort=False):
    """
    Save an input dictionary to a file
...
def dict_read(file_name='data', method='n'):
    """
    Read a saved dictionary in the input file
    """
    data = {}
    # Npy
    if method == 'n':
...
def dict_add(dictionary, key, value):
    """
    Add {KEY:VALUE} to a dictionary, if key does not already exist
    """
...
def dict_lookup(dictionary, key):
    """
    Search KEY in the input dictionary
    ...
    i.e. Similar to defaultdict behavior
    """
    value = 0
    if key not in dictionary:
